...
function TabProductos({ products, addProduct, updateProduct, removeProduct, toggleHidden, toggleFeatured }) {
  ...
  const [featuredMode, setFeaturedMode] = useState(() => (window.VETA_DB && window.VETA_DB.getSetting("featured_mode", "auto")) || "auto");
  ...
  return (
    <div className="adm-page">
      ...
      <div className="adm-toolbar">
        <span className="adm-hint" style={{ margin:0 }}>Destacados del inicio:</span>
        <div className="adm-pills">
          <button className={`adm-pill${featuredMode==="auto"?" adm-pill--on":""}`}
            onClick={() => changeFeaturedMode("auto")}
            title="El sitio elige 4-6 productos al azar cada día, priorizando los marcados Destacado: Sí">
            🎲 Aleatorio
          </button>
          <button className={`adm-pill${featuredMode==="manual"?" adm-pill--on":""}`}
        ...
        {featuredMode === "manual" && (
          <span className="adm-hint" style={{ margin:0 }}>
            Se muestran los marcados "Destacado: Sí" ({products.filter(p=>p.featured).length}/6 recomendado).
          </span>
        )}
      </div>
...
function orderDraftKey(fixedPhone) { return "veta_order_draft_" + (fixedPhone || "blank"); }
...
function loadOrderDraft(fixedPhone, fixedName) {
  ...
  return { phone: fixedPhone || "", customer: fixedName || "", city: "", neighborhood: "",
    address: "", aptRef: "", payment: "", recipient: "", items: "", notes: "" };
}
...
function OrderForm({ phone: fixedPhone, customerName: fixedName, onClose, onCreated }) {
  ...
  const hasContent = [customer, city, neighborhood, address, aptRef, payment, recipient, items, notes]
    .some(v => v.trim()) || (!fixedPhone && phone.trim());
  ...
  const discardDraft = () => { try { localStorage.removeItem(draftKey); } catch (e) {} };
  ...
  return (
    <div className="adm-modal-ov" onClick={onClose}>
      <div className="adm-crop adm-order-form" style={{ maxWidth: 480 }} onClick={e => e.stopPropagation()}>
        <header className="adm-order-form-hdr">
          ...
          <button type="button" className="adm-modal-x" onClick={onClose} title="Cerrar (tu borrador queda guardado)">×</button>
        </header>
        <form onSubmit={save} className="adm-order-form-body">
          <div className="adm-order-form-scroll">
            ...
            <div className="adm-form-field" style={{ marginTop: 10 }}>
              <label className="adm-lbl">Instrucciones de entrega <span className="adm-field-hint-inline">— opcional</span></label>
          ...
          <div className="adm-form-actions adm-order-form-footer">
            <button type="button" className="adm-btn adm-btn--ghost" onClick={cancelOrder} disabled={saving}>Cancelar pedido</button>
