/* VETA · capa de datos (Supabase)
   ...
   Antes vivía en window.VETA_DB; ahora es un módulo ESM que se importa
   explícitamente (`import { db } from "../lib/db.js"`). Sigue siendo un
   singleton: Supabase se inicializa una sola vez al importar. */
...
export const db = (function () {
  ...
  async function loadProducts() {
    ...
    if (error) {
      console.warn("[VETA_DB] products:", error.message);
      return;
    }
  ...
  async function loadStock() {
    ...
    if (error) {
      console.warn("[VETA_DB] stock:", error.message);
      return;
    }
    _stock = {};
    data.forEach((r) => {
  ...
  async function loadDiscountCodes() {
    ...
    if (error) {
      console.warn("[VETA_DB] discount_codes:", error.message);
      return;
    }
    _discountCodes = data || [];
  }
  ...
  async function loadCategories() {
    ...
    if (error) {
      console.warn("[VETA_DB] categories:", error.message);
      return;
    }
    _categories = data || [];
  }
  ...
  async function deleteStorageImage(url) {
    ...
    try {
      ...
      console.warn("[VETA_DB] deleteStorageImage:", e.message);
    }
  }
  ...
  async function validateCode(code, subtotal) {
    ...
    if (d.min_subtotal > 0 && subtotal < d.min_subtotal) {
      const minFmt = VETA_DATA.fmtPrice(d.min_subtotal);
  ...
  async function incrementCodeUses(code) {
    try {
      ...
      console.warn("[VETA_DB] incrementCodeUses:", e.message);
    }
  }
  ...
  async function notifyOrderCreated(orderId) {
    ...
    try {
      ...
      console.warn("[VETA_DB] notifyOrderCreated:", e.message);
    }
  }
  ...
  function _notifyChats(payload) {
    _chatSubs.forEach((fn) => {
      try {
        fn(payload);
      } catch {}
    });
  ...
  async function loadThreads() {
    ...
    if (error) {
      console.warn("[VETA_DB] threads:", error.message);
      return _threads;
    }
    _threads = {};
    (data || []).forEach((t) => {
  ...
  async function getConversationList(limit = 1000) {
    const { data, error } = await sb
      ...
      .limit(limit);
    if (error) {
      console.warn("[VETA_DB] conv list:", error.message);
      return [];
    }
    const byPhone = {};
    (data || []).forEach((m) => {
  ...
  async function getMessages(phone) {
    ...
    if (error) {
      console.warn("[VETA_DB] messages:", error.message);
      return [];
    }
    return data || [];
  }
  ...
  async function clearNeedsHuman(phone) {
    const { error } = await sb
      .from("wa_threads")
      .upsert(
        { phone, needs_human: false, updated_at: new Date().toISOString() },
        { onConflict: "phone" }
      );
    if (error) throw error;
